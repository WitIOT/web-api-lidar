    ...
    options: {
        ...
        plugins:{
            zoom: {
                zoom: {
                    wheel: {
                        enabled: true, // เปิดใช้งานการซูมด้วยวงล้อเมาส์
                    },
                    pinch: {
                        enabled: true, // เปิดใช้งานการซูมด้วยท่าทาง pinch บนอุปกรณ์ทัชสกรีน
                    },
                    mode: 'x' // หรือ 'y', 'xy' (ซูมในแนวแกน X, Y หรือทั้งสอง)
                },
                pan: {
                    enabled: true,
                    mode: 'x',
                    threshold: 10, // ความไวในการพาน
                    rangeMin: {
                        x: null,
                        y: null
                    },
                ...
                limits: {
                    x: {min: 'original', max: 'original'}, // จำกัดการซูมให้อยู่ในขอบเขตของข้อมูลเริ่มต้น
    ...
    fetch(`http://192.168.2.190:5000/data/ALiN/${selectedData}`)
        ...
        .then(data => {
            ...
            callback(mplCalValues, mplDisValues, ocCalValues, ocDisValues);
             renderFusionCharts(ocCalValues, ocDisValues);
        })
        .catch(error => console.error('Error fetching data:', error));
}
